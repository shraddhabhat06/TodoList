...
const addTodo=(e)=>{
  ...
  if(todo){
    ...
    <input type="text" placeholder ="task"class="todo-text" value=${todo} disabled>
...
const getTodo=(e)=>{
  ...
  if(tasks){
    ...
    tasks.forEach((task)=>{
      ...
      todoItem.innerHTML =` <input type="checkbox" class="todo-check" ${task.completed ? "checked":""}>
      <input type="text" placeholder ="task"class="todo-text ${task.completed ? "strike" : ""}" value=${task.todo}  disabled>
      <button class="todo-edit">Edit</button>
      <button class="todo-delete">Delete</button>`;
      todoOutput.appendChild(todoItem);
